...
import { GameField } from '../gameCycle/gameContainer';
import { ArtistData } from '../gameCycle/data';
import { MainScreen } from '../mainScreen/MainScreen';

export class ArtistScreen extends BaseAnimatedComponent {
  constructor(parentElement){
    ...
    this.artistScoreOpenBtn.element.onclick = () => {
      this.screenEnd(new ArtistScore(document.body))
    }

    this.settingOpen.element.onclick = () => {
      this.screenEnd(new SettingScreen(document.body, 'Artist screen'))
    }
  ...
  start(){
    ...
    for(let i = 1; i <= 12; i++) {
      let categoryCard = new ArtistQuizCategoryCard(this.categorySelectContainer.element, this.categoryIndex);
      ...
      if(!filterAnswers) {
        ...
        // categoryCard.element.classList.add('play-again');
...
export class ArtistQuizCategoryCard extends BaseComponent {
  constructor(parentElement, category){
    ...
    this.setBackgroundImage(category * 5);

    this.animation = new Animation();
    this.cardImage.element.addEventListener('click',() => {
      this.animation.nextScreenAnimation(this.element.parentElement.parentElement, new GameField(document.body, category));
    }, {once: true})
  }
...
export class ArtistScore extends BaseAnimatedComponent {
  constructor(parentElement){
    super(parentElement, 'div', ['artis-score-screen', 'container']);

    let headingDiscription = new BaseComponent(this.element, 'h2', ['score-discription'], 'Artist quiz score')
    let categoryWrapper = new BaseComponent(this.element, 'div', ['score-categories-wrapper']);

    this.artistScore = new Storage('Artist');
    this.data = new ArtistData();
    this.dataChunk = this.data.chunksQuestionsByAuthor;
    for(let i = 1; i <= 12; i++) {
...
export class ImageDiscriptionData extends BaseComponent {
  constructor(parentElement, data){
    super(parentElement, 'div', ['image-discription-data']);

    this.author = new BaseComponent(this.element, 'p', ['image-discription--author'], `${data.author}`);

    this.name = new BaseComponent(this.element, 'p', ['image-discription--author'], `"${data.name}"`);

    this.year = new BaseComponent(this.element, 'p', ['image-discription--author'], `${data.year} год`);
  }
}
